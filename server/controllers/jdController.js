...
exports.createJD = async (req, res) => {
    try {
        ...
        db.query(
            ...
            (err, result) => {
                if (err) {
                    console.error("JD insert error:", err);
        ...
        return res.status(500).json({
            message: "Job description processing failed"
        });
    }
};
...
exports.getRankings = (req, res) => {
    try {
        ...
        // Verify that JD belongs to logged-in HR
        const checkJdSql = `
            SELECT jd_id
        ...
        db.query(
            ...
            (jdErr, jdResults) => {
                if (jdErr) {
                    ...
                    return res.status(500).json({
                        message:
                            "Failed to verify job description"
                    });
                }
                ...
                /*
                    ...
                    for each candidate for this JD.
                */
                const rankingSql = `
                    SELECT
                        sr.result_id,
